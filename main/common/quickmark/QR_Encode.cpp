...
QImage CQR_Encode::QRImage(int nLevel, int nVersion, bool bAutoExtent, int nMaskingNo, LPCWSTR lpsSource, int ncSource /* = 0 */)
{
    QImage img;
    if (!EncodeData(0, 3, false, false, lpsSource))
        return img;
    int symbleSize = m_nSymbleSize + (QR_MARGIN * 2);
    img = QImage(symbleSize, symbleSize, QImage::Format_RGB32);
    ...
    for (int i = 0; i < m_nSymbleSize; ++i)
    {
        for (int j = 0; j < m_nSymbleSize; ++j)
        {
            ...
            {
                img.setPixel(i + QR_MARGIN, j + QR_MARGIN, qRgb(0, 0, 0));
            }
            else
            {
                img.setPixel(i + QR_MARGIN, j + QR_MARGIN, qRgb(255, 255, 255));
            }
        }
    }
    return img;
