...
{
  savePixmapAction = new QAction(tr("saveSelect"),this);
...
{
  QString fileName;
  fileName = QFileDialog::getSaveFileName(this,tr("savePic"),QDir::currentPath(),tr("Images (*.jpg *.png *.bmp)"));
  if(fileName.isNull())
    return;
...
void fullScreenWidget::drawTipsText()
{
  ...
  QString strTipsText = QString(tr("screenShotTips"));
  //鼠标拖动进行截屏;截屏区域内右键保存;\n截屏区域外右键取消;ESC退出;
