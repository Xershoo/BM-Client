...
bool CUDPClinet::InitUDPClient(long nUserID,int nType)
{
	m_nType = nType;
	m_nUserID = nUserID;
	m_lastGetLiveTime = 0;
...
int CUDPClinet::parsePacketAndRead(unsigned char* pBuf,unsigned int nBufLen)
{
	...
	if(pInBufer)
	{
		...
		char szbuf[256] = {'/0'};
		sprintf(szbuf,"recv data packet no %d\n",pInBufer->getSeqNo());
...
void CUDPClinet::recv_loop()
{
	...
	while(!m_recvThread.GetStop())
	{
		if(m_sendSocket && m_nType == UDPPULL)
		{
			...
			if(m_InBufPacket.CheckGiveUPPacket(m_nSSRC,nCurTimePts,szMsg) > 0)
			{
				sprintf(szTempMsg,"%s\n",szMsg);
				OutputDebugString(szTempMsg);
		...
		if(m_recvSocket > 0 )
		{
			...
			if(nSize > 0)
			{
			   ...
			   {
					if(m_InBufPacket.getLastReavPacketSeqNo() != m_InBufPacket.getRecvDataSeqNo(m_recvbuf))
						m_InBufPacket.putBufToInPacket(m_recvbuf,nSize);
			   }
			}
