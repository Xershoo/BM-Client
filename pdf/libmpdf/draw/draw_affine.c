...
{
	if (fabs(m->b) < FLT_EPSILON && fabs(m->c) < FLT_EPSILON)
	{
		if (m->a > 0)
		{
			float f;
	...
	else if (fabs(m->a) < FLT_EPSILON && fabs(m->d) < FLT_EPSILON)
	{
		if (m->b > 0)
		{
			float f;
...
fz_paint_image_imp(fz_pixmap *dst, fz_bbox scissor, fz_pixmap *shape, fz_pixmap *img, fz_matrix ctm, byte *color, int alpha)
{
	...
	if (!fz_is_rectilinear(ctm))
		dolerp = 1;
	if (sqrt(ctm.a * ctm.a + ctm.b * ctm.b) > img->w)
		dolerp = 1;
	if (sqrt(ctm.c * ctm.c + ctm.d * ctm.d) > img->h)
		dolerp = 1;

	/* except when we shouldn't, at large magnifications */
	if (!img->interpolate)
	{
		if (sqrt(ctm.a * ctm.a + ctm.b * ctm.b) > img->w * 2)
			dolerp = 0;
		if (sqrt(ctm.c * ctm.c + ctm.d * ctm.d) > img->h * 2)
			dolerp = 0;
	}
