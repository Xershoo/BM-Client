...
fz_evict_glyph_cache(fz_context *ctx)
{
	...
	{
		key = (fz_glyph_key*)fz_hash_get_key(ctx, cache->hash, i);
		if (key->font)
			fz_drop_font(ctx, key->font);
		pixmap = (fz_pixmap*)fz_hash_get_val(ctx, cache->hash, i);
		if (pixmap)
...
fz_render_glyph(fz_context *ctx, fz_font *font, int gid, fz_matrix ctm, fz_colorspace *model, fz_bbox scissor)
{
	...
	ctm.e = floor(ctm.e) + key.e / 256.0f;
	ctm.f = floor(ctm.f) + key.f / 256.0f;
