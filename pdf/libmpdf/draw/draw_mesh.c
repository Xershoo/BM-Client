...
#else
#define my_sinf(x) sin(x)
#define my_cosf(x) cos(x)
#define my_atan2f(x,y) atan2(x,y)
#endif
...
fz_paint_triangle(fz_pixmap *pix, float *av, float *bv, float *cv, int n, fz_bbox bbox)
{
	...
	for (i = 0; i < len; i++)
	{
		gel[i][0] = floor(poly[i][0] + 0.5f) * 65536; /* trunc and fix */
		gel[i][1] = floor(poly[i][1] + 0.5f);	/* y is not fixpoint */
		for (k = 2; k < n; k++)
