	...
	float x1, float y1)
{
	...
	r = fabs(s->linewidth);
	theta = 2 * (float)M_SQRT2 * sqrt(s->flatness / r);
	th0 = atan2(y0, x0);
	th1 = atan2(y1, x1);
...
fz_add_line_join(struct sctx *s, fz_point a, fz_point b, fz_point c, int join_under)
{
	...
	scale = linewidth / sqrt(len0);
	dlx0 = dy0 * scale;
	dly0 = -dx0 * scale;

	scale = linewidth / sqrt(len1);
	dlx1 = dy1 * scale;
	dly1 = -dx1 * scale;
	...
	if (linejoin == FZ_LINEJOIN_MITER_XPS)
	{
		...
		else
		{
			...
			dmx *= scale;
			dmy *= scale;
			k = (scale - linewidth * miterlimit / sqrt(dmr2)) / (scale - 1);
...
fz_add_line_cap(struct sctx *s, fz_point a, fz_point b, fz_linecap linecap)
{
	...
	{
		int i;
		int n = ceil((float)M_PI / (2.0f * (float)M_SQRT2 * sqrt(flatness / linewidth)));
		float ox = b.x - dlx;
		float oy = b.y - dly;
		for (i = 1; i < n; i++)
		{
...
fz_add_line_dot(struct sctx *s, fz_point a)
{
	float flatness = s->flatness;
	float linewidth = s->linewidth;
	int n = ceil((float)M_PI / ((float)M_SQRT2 * sqrt(flatness / linewidth)));
...
fz_dash_moveto(struct sctx *s, fz_point a, fz_linecap start_cap, fz_linecap end_cap)
{
	...
	if (s->toggle)
	{
		fz_stroke_flush(s, (fz_linecap)s->cap, (fz_linecap)end_cap);
		s->cap = start_cap;
...
fz_dash_lineto(struct sctx *s, fz_point b, int dash_cap, int from_bezier)
{
	...
	while (total - used > s->dash_list[s->offset] - s->phase)
	{
		...
		{
			fz_stroke_flush(s, (fz_linecap)s->cap, (fz_linecap)dash_cap);
			s->cap = dash_cap;
...
fz_flatten_dash_path(fz_gel *gel, fz_path *path, fz_stroke_state *stroke, fz_matrix ctm, float flatness, float linewidth)
{
	...
	fz_stroke_flush(&s, (fz_linecap)s.cap, stroke->end_cap);
}
