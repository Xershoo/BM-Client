...
lanczos2(fz_scale_filter *filter, float x)
{
	if (x >= 2)
		return 0;
	return sin(M_PI*x) * sin(M_PI*x/2) / (M_PI*x) / (M_PI*x/2);
}
...
lanczos3(fz_scale_filter *filter, float f)
{
	if (f >= 3)
		return 0;
	return sin(M_PI*f) * sin(M_PI*f/3) / (M_PI*f) / (M_PI*f/3);
}
...
new_weights(fz_context *ctx, fz_scale_filter *filter, int src_w, float dst_w, int patch_w, int n, int flip, int patch_l)
{
	...
	if (src_w > dst_w)
	{
		...
		max_len = (int)ceil((2 * filter->width * src_w)/dst_w);
		if (max_len > src_w)
			max_len = src_w;
	}
	else
	 ...
	 */
	weights = (fz_weights*)fz_malloc(ctx, sizeof(*weights)+(max_len+3)*(patch_w+1)*sizeof(int));
	if (!weights)
		return NULL;
	weights->count = -1;
...
fz_scale_pixmap(fz_context *ctx, fz_pixmap *src, float x, float y, float w, float h, fz_bbox *clip)
{
...
#endif /* SINGLE_PIXEL_SPECIALS */
	{
		...
		fz_try(ctx)
		{
			temp = (int*)fz_calloc(ctx, temp_span*temp_rows, sizeof(int));
		}
		fz_catch(ctx)
		{
