...
extern struct pdf_document *pdf_open_document_w(fz_context *ctx, char *filename);
...
fz_document *
fz_open_document_w(fz_context *ctx, wchar_t *filename)
{
	return (fz_document*) pdf_open_document_w(ctx, filename);
}

fz_document *
fz_open_document_with_data(fz_context *ctx, unsigned char* data,int len)
{
	return (fz_document*) pdf_open_document_with_data(ctx, data,len);
}
