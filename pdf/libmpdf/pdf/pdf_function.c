		...
		case PS_OPERATOR:
			...
			case PS_OP_ABS:
				if (ps_is_type(st, PS_INT))
					...
					ps_push_real(st, fabs(ps_pop_real(st)));
				break;
			...
			case PS_OP_ROUND:
				if (!ps_is_type(st, PS_INT)) {
					r1 = ps_pop_real(st);
					ps_push_real(st, (r1 >= 0) ? floor(r1 + 0.5f) : ceil(r1 - 0.5f));
				}
				break;
			...
			case PS_OP_TRUNCATE:
				if (!ps_is_type(st, PS_INT)) {
					r1 = ps_pop_real(st);
					ps_push_real(st, (r1 >= 0) ? floor(r1) : ceil(r1));
				}
				break;
...
resize_code(fz_context *ctx, pdf_function *func, int newsize)
{
	if (newsize >= func->u.p.cap)
	{
		...
		func->u.p.code = (psobj*)fz_resize_array(ctx, func->u.p.code, new_cap, sizeof(psobj));
...
load_sample_func(pdf_function *func, pdf_document *xref, pdf_obj *dict, int num, int gen)
{
	...
	func->u.sa.samples = (float*)fz_malloc_array(ctx, samplecount, sizeof(float));
...
load_stitching_func(pdf_function *func, pdf_document *xref, pdf_obj *dict)
{
		...
		fz_throw(ctx, "stitching function has no input functions");
	{
		...
		func->u.st.funcs = (pdf_function**)fz_malloc_array(ctx, k, sizeof(pdf_function*));
		func->u.st.bounds =(float*) fz_malloc_array(ctx, k - 1, sizeof(float));
		func->u.st.encode = (float*)fz_malloc_array(ctx, k * 2, sizeof(float));
...
pdf_load_function(pdf_document *xref, pdf_obj *dict, int in, int out)
{
	...
	if ((func = (pdf_function*)pdf_find_item(ctx, pdf_free_function_imp, dict)))
	{
		return func;
	}
