...
pdf_show_pattern(pdf_csi *csi, pdf_pattern *pat, fz_rect area, int what)
{
	...
	x0 = floor((area.x0 - pat->bbox.x0) / pat->xstep + 0.001);
	y0 = floor((area.y0 - pat->bbox.y0) / pat->ystep + 0.001);
	x1 = ceil((area.x1 - pat->bbox.x0) / pat->xstep - 0.001);
	y1 = ceil((area.y1 - pat->bbox.y0) / pat->ystep - 0.001);
