...
pdf_document *
pdf_open_document_no_run_w(fz_context *ctx, const wchar_t *filename)
{
	fz_stream *file = NULL;
	pdf_document *doc;
...
pdf_document *
pdf_open_document_no_run_with_data(fz_context *ctx, unsigned char *data, int len)
{
	fz_stream *file = NULL;
	pdf_document *doc;
	...
	fz_try(ctx)
	{
		file = fz_open_memory(ctx,data,len);
		doc = pdf_new_document(file);
		pdf_init_document(doc);
	}
	fz_always(ctx)
	...
	fz_catch(ctx)
	{
		fz_throw(ctx, "cannot load document");
	}
	return doc;
}
