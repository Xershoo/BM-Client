...
pdf_document *
pdf_open_document_with_data(fz_context *ctx,unsigned char *data, int len)
{
	pdf_document *doc = pdf_open_document_no_run_with_data(ctx, data,len);
	doc->super.run_page = pdf_run_page_shim;
	return doc;
}
...
pdf_document *
pdf_open_document_w(fz_context *ctx, const wchar_t *filename)
{
	pdf_document *doc = pdf_open_document_no_run_w(ctx, filename);
	doc->super.run_page = pdf_run_page_shim;
	return doc;
}
