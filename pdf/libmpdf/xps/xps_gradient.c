	...
	xml_element *root, int spread)
{
	...
	ma = fz_maxi(ma, ceil(hypot(area.x0 - x0, area.y0 - y0) / xrad));
	ma = fz_maxi(ma, ceil(hypot(area.x1 - x0, area.y0 - y0) / xrad));
	ma = fz_maxi(ma, ceil(hypot(area.x0 - x0, area.y1 - y0) / xrad));
	ma = fz_maxi(ma, ceil(hypot(area.x1 - x0, area.y1 - y0) / xrad));
	...
	xml_element *root, int spread)
{
	...
	mi = floor(k); ma = ceil(k);
	k = ((area.x1 - p1.x) * x + (area.y0 - p1.y) * y) / (x * x + y * y);
	mi = fz_mini(mi, floor(k)); ma = fz_maxi(ma, ceil(k));
