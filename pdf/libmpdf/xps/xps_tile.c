	...
	void (*func)(xps_document*, fz_matrix, fz_rect, char*, xps_resource*, xml_element*, void*), void *user)
{
	...
	if (fabs(viewport.x1 - viewport.x0) < 0.01f || fabs(viewport.y1 - viewport.y0) < 0.01f)
		fz_warn(doc->ctx, "not drawing tile for viewport size %.4f x %.4f", viewport.x1 - viewport.x0, viewport.y1 - viewport.y0);
	else if (fabs(viewbox.x1 - viewbox.x0) < 0.01f || fabs(viewbox.y1 - viewbox.y0) < 0.01f)
		fz_warn(doc->ctx, "not drawing tile for viewbox size %.4f x %.4f", viewbox.x1 - viewbox.x0, viewbox.y1 - viewbox.y0);

	/* some sanity checks on the viewport/viewbox size */
	if (fabs(viewport.x1 - viewport.x0) < 0.01f) return;
	if (fabs(viewport.y1 - viewport.y0) < 0.01f) return;
	if (fabs(viewbox.x1 - viewbox.x0) < 0.01f) return;
	if (fabs(viewbox.y1 - viewbox.y0) < 0.01f) return;
